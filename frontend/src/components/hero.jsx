...
export const getCount = () => {
  ...
  let totalQuantity = 0;
  //  console.log(getAllCarts[0].productQuantity)
...
function Hero() {
  ...
  const fetchAllProducts = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_APP_URL}/:userId/all-products`,
        {
          method: "GET",
        }
      );
  ...
  */
  ...
  const handleAddToCart = (productId, productPhoto, productTitle, productPrice) => {
    ...
    // Call exportPlusClick to handle additional logic if necessary
  ...
  return (
    <>
      <div className="grid grid-cols-1  md:grid-cols-3 maxWidth lg:grid-cols-4 gap-4">
        ...
        {allProducts.length > 0 ? (
          <>
            {allProducts.map((allProduct) => (
              ...
              <div
                key={allProduct._id}
                className="relative flex w-full  wid flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md"
              >
                <a
                ...
                <div className="mt-4 px-5 pb-5">
                  ...
                  <button onClick={() => handleAddToCart(allProduct._id, allProduct.productPhoto, allProduct.productTitle, allProduct.productPrice)} >
                    {/* <button onClick={()=> handleBuyProduct(allProduct._id,allProduct.productPrice)}> */}
